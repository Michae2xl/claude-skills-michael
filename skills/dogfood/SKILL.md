...
The `agent-browser` CLI must be installed. If not available, run:
...
**Important -- Manus sandbox compatibility:** The Playwright-bundled Chromium may not work in this sandbox. Instead, connect to the pre-existing Chromium instance via CDP:

```bash
# Use --cdp 9222 flag on ALL agent-browser commands
agent-browser --cdp 9222 open https://example.com
agent-browser --cdp 9222 snapshot -i
agent-browser --cdp 9222 screenshot output.png
```

When using sessions, combine `--cdp 9222` with `--session`:

```bash
agent-browser --cdp 9222 --session {SESSION} open {TARGET_URL}
```
...
**In the Manus sandbox, always add `--cdp 9222` to connect to the existing Chromium instance.** All commands below assume this flag is used.
...
```bash
cp /home/ubuntu/skills/dogfood/templates/dogfood-report-template.md {OUTPUT_DIR}/report.md
```
